...
const storeAnswer = async ({ response, params, user }) => {
  ...
  dataSolution.specificTopic = await topicsService.listSpecificTopic(
    params.tId,
  );
  ...
  if (correctness.is_correct == true) {
    ...
    response.redirect(
      "/quiz/" + `${params.tId}` + "/questions/" + `${params.qId}` +
        "/incorrect",
    );
  }
};
...
export { listTopics, pickRandomQuestion, quizAnswers, solution, storeAnswer };
