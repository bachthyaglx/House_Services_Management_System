...
const displayListRoom = async (firstname, sex) => {
  const table2 = await executeQuery(
    `SELECT city, zipcode, address FROM rooms AS t WHERE apartment_id IN (SELECT apartment_id FROM application WHERE user_id IN (SELECT id FROM users AS c WHERE firstname=$firstname)) AND (t.sex='' OR t.sex=$sex)`,
    {
      firstname: firstname,
      sex: sex,
    },
